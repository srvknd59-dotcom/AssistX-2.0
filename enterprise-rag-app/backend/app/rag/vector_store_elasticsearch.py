"""An Elasticsearch-backed vector store — the same interface as ChromaVectorStore
in vector_store.py, but backed by a real Elasticsearch index with a dense_vector
field and kNN search, the same approach the production chat_service.py/ingestor.py
in this repo use.

Elasticsearch is a separate server process (unlike Chroma, which is embedded),
so this requires Elasticsearch running and reachable at ES_URL before the
backend starts. See backend/README_ELASTICSEARCH.md for local setup on Windows.
"""
